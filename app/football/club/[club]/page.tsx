...
function favoriteForClub(clubName: string): FavoriteCandidate {
  return { kind: "participant", entityId: clubEntityId(clubName), label: `${clubName} (Football)` };
}
  ...
  return (
    <main id="main-content" className="v2-calendar">
      ...
      <section className="v2-calendar-hero" aria-labelledby="club-title">
        ...
        <p className="v2-hero-copy">Upcoming matches and verified official broadcasters for {clubName}.</p>
        <FavoriteButton favorite={favorite} />
        {aliases.length > 0 ? <p className="v2-signature">Also known as: {aliases.join(" · ")}</p> : null}
      </section>
      ...
      <section className="v2-results" aria-labelledby="upcoming-title">
        ...
        {upcoming.length === 0 ? (
          <div className="v2-empty-state" role="status"><h3>No upcoming match currently confirmed</h3><p>WatchTVSport will show new fixtures here as soon as they are confirmed.</p></div>
        ) : (
          <div className="v2-event-list">
            {upcoming.map((event) => (
              <article className="v2-event-card" key={event.id}>
                <div className="v2-event-main">
                  <p className="v2-event-competition"><Link href={competitionHref(event)}>{event.competition}</Link></p>
                  <h3>{event.title}</h3>
