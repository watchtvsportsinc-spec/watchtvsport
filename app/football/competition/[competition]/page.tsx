...
export async function generateStaticParams() {
  return Array.from(new Set(getAllEvents().filter((event) => event.sport === "football").map((event) => event.competitionSlug))).map((competition) => ({ competition }));
}
...
export default async function CompetitionPage({ params }: PageProps) {
  ...
  const clubs = Array.from(
    new Map(
      events.flatMap((event) => [event.participant1, event.participant2]
  ...
  return (
    <main id="main-content" className="v2-calendar">
      ...
      <section className="v2-calendar-hero" aria-labelledby="competition-title">
        ...
        <p className="v2-hero-copy">Upcoming fixtures, participating teams and verified official broadcasters for {name}.</p>
      ...
      {clubs.length > 0 ? (
        <section className="v2-results" aria-labelledby="participants-title">
          ...
          <div className="v2-entity-grid">
            {clubs.map((club) => <Link key={club.id} href={`/football/club/${clubSlug(club.name)}`}><strong>{club.name}</strong><span>Club</span></Link>)}
          </div>
        </section>
      ) : null}
