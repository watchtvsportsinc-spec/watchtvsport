...
export async function generateMetadata({params}:PageProps):Promise<Metadata>{const{sport}=await params;if(REDIRECTS[sport])return{title:"Redirecting | WatchTVSport",robots:{index:false,follow:true},alternates:{canonical:REDIRECTS[sport]}};if(!SUPPORTED.has(sport))return{title:"Sport not found | WatchTVSport",robots:{index:false,follow:false}};return buildSportHubMetadata(sport);}
export default async function SportPage({params}:PageProps){const{sport}=await params;if(REDIRECTS[sport])redirect(REDIRECTS[sport]);if(!SUPPORTED.has(sport))notFound();return <SportHubPage sport={sport}/>;}
