...
export default async function LeagueCompetitionPage({competition}:{competition:string}){
  ...
  return <main id="main-content" className={styles.page}>
    ...
    <section className={styles.hero} style={{backgroundImage:"linear-gradient(90deg,rgba(4,13,23,.98),rgba(4,16,28,.75)),url('/sports/football.webp')"}}>
      <p className={styles.eyebrow}>Domestic league · {profile.seasonLabel}</p><h1>{profile.displayName}</h1><p className={styles.heroCopy}>{profile.heroCopy}</p>
    ...
    <section id="participants" className={styles.section}><div className={styles.sectionHeading}><div><p>{profile.seasonLabel} league</p><h2>{teams.length} clubs</h2></div><span>{teams.length}</span></div><div className={styles.participants}>{teams.map(team=><Link className={styles.participant} key={team.slug} href={`/football/club/${team.slug}`}><ParticipantSportVisual sport="football" label={team.name} visual={team.visual} size="sm"/><span><strong>{team.name}</strong><small>Club profile →</small></span></Link>)}</div></section>
