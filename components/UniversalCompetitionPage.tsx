...
export default async function UniversalCompetitionPage({sport,competition}:{sport:string;competition:string}){
 ...
 const scheduleItems:CompetitionScheduleItem[]=events.map(event=>{const offers=event.broadcasts.filter(b=>b.coverageStatus==="confirmed");const p1Key=eventParticipantKey(event.participant1);const p2Key=eventParticipantKey(event.participant2);return{id:event.id,detailPath:event.detailPath,title:event.title,eventDate:event.eventDate,stage:event.stage,status:event.status,sport,participant1:event.participant1?{name:event.participant1.name,countryCode:event.participant1.countryCode,visual:event.participant1.visualProfile,logoUrl:p1Key?participantLogos[p1Key]?.url:undefined}:undefined,participant2:event.participant2?{name:event.participant2.name,countryCode:event.participant2.countryCode,visual:event.participant2.visualProfile,logoUrl:p2Key?participantLogos[p2Key]?.url:undefined}:undefined,confirmed:offers.length,freeCountries:new Set(offers.filter(b=>b.access==="Free").map(b=>b.countryCode)).size,paidCountries:new Set(offers.filter(b=>b.access==="Paid").map(b=>b.countryCode)).size};});
