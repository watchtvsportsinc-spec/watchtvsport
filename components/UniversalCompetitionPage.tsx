...
export default async function UniversalCompetitionPage({sport,competition}:{sport:string;competition:string}){
 ...
 return <main id="main-content" className={styles.page}>
  ...
  <section className={styles.hero} style={{backgroundImage:`linear-gradient(90deg,rgba(4,13,23,.98),rgba(4,16,28,.75)),url('${backdrop}')`}}><p className={styles.eyebrow}>{category.replace(/-/g," ")}{season?` · ${season}`:""}</p><h1>{name}</h1><p className={styles.heroCopy}>{heroCopy}</p><div className={styles.heroNav}><a href="#schedule">Schedule</a>{teams.length?<a href="#participants">Clubs</a>:null}<a href="#about">About</a><FavoriteButton favorite={favorite}/></div></section>
  ...
  {teams.length?<section id="participants" className={styles.section}><div className={styles.sectionHeading}><div><p>{leagueProfile?`${season} league`:"Participants"}</p><h2>{leagueProfile?`${teams.length} clubs`:"Teams & participants"}</h2></div><span>{teams.length}</span></div><div className={styles.participants}>{teams.map(team=><Link className={styles.participant} key={team.slug} href={participantPath(sport,team)}><ParticipantSportVisual sport={sport} label={team.name} countryCode={team.countryCode} visual={team.visual} size="sm"/><span><strong>{team.name}</strong><small>Club profile →</small></span></Link>)}</div></section>:null}
