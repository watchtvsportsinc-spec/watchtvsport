...
const CATEGORY_LABELS:Record<CompetitionCategory,string>={continental:"European & continental competitions","domestic-league":"Domestic leagues","domestic-cup":"Domestic cups","international":"International competitions","grand-slam":"Grand Slams","tour":"Tour events","league":"Leagues","championship":"Championships","organization":"Organizations","other":"Other competitions"};
...
export default async function SportHubPage({sport,canonical}:{sport:string;canonical?:string}){
 ...
 const map=new Map<string,{slug:string;name:string;category:CompetitionCategory;eventCount:number;next:string|null;season?:string;available:boolean}>();
 for(const c of permanent)map.set(c.slug,{slug:c.slug,name:c.displayName||c.name,category:(c.competitionType as CompetitionCategory)||classifyCompetition(sport,c.slug),eventCount:0,next:null,season:c.seasonLabel,available:true});
