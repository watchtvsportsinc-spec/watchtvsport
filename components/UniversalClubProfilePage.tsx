    ...
    <main id="main-content" className={`v2-calendar ${styles.page}`}>
        ...
        <div className={styles.heroContent}>
          ...
          {nextMatch ? (
            <aside className={styles.heroNext} aria-label={`${clubName} next game`}>
              ...
              <span className={styles.heroNextCompetition}>{nextMatch.competition}{nextMatch.stage ? ` · ${nextMatch.stage}` : ""}</span>
      ...
      <div className={styles.layout}>
        <div className={styles.mainColumn}>
          {nextMatch ? (
            <section className={`${styles.section} ${styles.featureSection}`} aria-labelledby="next-match-title">
              ...
              <article className={styles.nextMatch} style={{ backgroundImage: `linear-gradient(180deg,rgba(3,11,19,.44),rgba(3,11,19,.94)),url('${fallbackBackdrop}')` }}>
                <div className={styles.nextMatchInner}>
                  ...
                  <div className={styles.versusRow}>
                    <div>
                      {nextMatch.participant1 ? <ParticipantSportVisual sport={sport} label={nextMatch.participant1.name} countryCode={nextMatch.participant1.countryCode} visual={nextMatch.participant1.visualProfile} size="lg" /> : <span className={styles.tbcVisual}>TBC</span>}
                      <strong>{nextMatch.participant1?.name ?? "TBC"}</strong>
                    </div>
                    <span className={styles.vs}>VS</span>
                    <div>
                      {nextMatch.participant2 ? <ParticipantSportVisual sport={sport} label={nextMatch.participant2.name} countryCode={nextMatch.participant2.countryCode} visual={nextMatch.participant2.visualProfile} size="lg" /> : <span className={styles.tbcVisual}>TBC</span>}
                      <strong>{nextMatch.participant2?.name ?? "TBC"}</strong>
                    </div>
                  </div>
                  <div className={styles.matchMeta}>
                    <LocalTime date={nextMatch.eventDate} />
                    {nextMatch.venue ? <><span>•</span><span>{nextMatch.venue}</span></> : null}
                    <span>•</span><span>{nextMatch.broadcasts.filter((b) => b.coverageStatus === "confirmed").length} confirmed listings</span>
                  </div>
                  <div className={styles.matchActions}><Link className={styles.primaryButton} href={nextMatch.detailPath}>Broadcasters & match details →</Link></div>
                </div>
              </article>
            </section>
          ) : null}
        ...
        <aside className={styles.sidebar}>
          <section className={styles.sideCard} id="team-info">
          ...
          <section className={styles.sideCard} id="competitions">
            ...
            ) : <p>No current competition schedule has been imported yet.</p>}
          </section>
          ...
          <section className={`${styles.sideCard} ${styles.guideCard}`}>
            ...
            <p>Broadcasters are shown on each match page by country. WatchTVSport does not invent missing coverage.</p>
            {nextMatch ? <Link href={nextMatch.detailPath}>Open next match page →</Link> : <Link href="/events">Browse current events →</Link>}
          </section>
        </aside>
      </div>
    </main>
