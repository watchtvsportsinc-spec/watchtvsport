...
-- Public, read-only JSON contract consumed by the Next.js server.
-- Apply after supabase-v2-foundation-migration.sql.
...
AS $function$
  ...
  FROM (
      ...
      jsonb_strip_nulls(jsonb_build_object(
        ...
        'detailPath', CASE
          WHEN page_url.url_path IS NOT NULL THEN
            page_url.url_path || '?event=' || e.id::text
    ...
    LEFT JOIN LATERAL (
      SELECT jsonb_agg(
        offer.payload
        ORDER BY offer.territory_name, offer.broadcaster_name,
          offer.broadcast_type, offer.offer_id
      ) AS payload
      FROM (
        SELECT
          eb.id AS offer_id,
          ...
          jsonb_strip_nulls(jsonb_build_object(
            ...
            'affiliateUrl', CASE
              WHEN eb.affiliate_url ~ '^https://' THEN eb.affiliate_url
              ELSE NULL
            END,
            'sourceName', COALESCE(eb.source_name, br.source_name),
            'sourceUrl', CASE
              WHEN COALESCE(eb.source_url, br.source_url) ~ '^https://'
                THEN COALESCE(eb.source_url, br.source_url)
              ELSE NULL
            END,
...
COMMENT ON FUNCTION public.get_public_events_v2() IS
  'Returns published, confirmed events and broadcast offers without internal provider identifiers.';
