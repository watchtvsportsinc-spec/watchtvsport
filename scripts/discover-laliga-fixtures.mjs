...
const CALENDAR_TOKEN_RE = new RegExp(`Matchday\\s+(\\d+)\\s*\\|\\s*(\\d{2})\\.(\\d{2})\\.(\\d{4})|(${TEAM_RE})`, "g");
...
export function extractCalendarFixtures(html) {
  const text = decodeEmbedded(html);
  const fixtures = [];
  let gameweek = null;
  let nominalDate = null;
  let pendingTeams = [];

  for (const match of text.matchAll(CALENDAR_TOKEN_RE)) {
    if (match[1]) {
      gameweek = Number(match[1]);
      nominalDate = `${match[4]}-${match[3]}-${match[2]}`;
      pendingTeams = [];
      continue;
    }
    if (!gameweek || !match[5]) continue;
    const team = canonicalTeam(match[5]);
    if (!team) continue;
    if (pendingTeams.at(-1) === team) continue;
    pendingTeams.push(team);
    if (pendingTeams.length === 2) {
      const [home, away] = pendingTeams;
      if (home !== away) fixtures.push({ competition: "laliga", season: "2026-27", gameweek, localDate: nominalDate, localTime: null, timezone: "Europe/Madrid", home, away, sourceUrl: CALENDAR_URL, verificationStatus: "confirmed", datePrecision: "matchday_nominal" });
      pendingTeams = [];
    }
  }
  return fixtures;
...
export async function discoverLaLigaFixtures({ fetchImpl = fetch } = {}) {
  const calendarHtml = await fetchText(fetchImpl, CALENDAR_URL);
  const calendarFixtures = extractCalendarFixtures(calendarHtml);
  const baselineValidation = validateFixtures(calendarFixtures);
  if (!baselineValidation.ok) return { source: "LALIGA official website", sourceUrl: CALENDAR_URL, ...baselineValidation, count: calendarFixtures.length, confirmedKickoffs: 0, diagnostics: { htmlBytes: calendarHtml.length, hasMatchday: /Matchday/i.test(decodeEmbedded(calendarHtml)), hasBarcelona: /FC Barcelona/i.test(decodeEmbedded(calendarHtml)) }, fixtures: calendarFixtures };
  ...
  return { source: "LALIGA official website", sourceUrl: CALENDAR_URL, ...validation, count: fixtures.length, confirmedKickoffs: fixtures.filter((fixture) => fixture.localTime).length, fixtures };
}
