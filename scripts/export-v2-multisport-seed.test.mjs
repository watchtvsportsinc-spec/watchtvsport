...
test("combined V2 seed contains UCL, complete F1 weekends and UFC fight cards", async () => {
  ...
  for (const sessions of sessionsByWeekend.values()) {
    ...
    assert.deepEqual(sessions.map((session) => session.sequenceNumber).sort((a, b) => a - b), [1, 2, 3, 4, 5]);
