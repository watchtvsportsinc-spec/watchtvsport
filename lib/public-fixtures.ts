...
export type PublicFixture = {
  ...
  seasonLabel?:string; matchweek?:number; windowStart?:string; windowEnd?:string; stage?:string;
...
function isSupabaseDataEnabled(){return process.env.WATCHTVSPORT_DATA_SOURCE?.trim()==="supabase";}
...
async function loadFixturePage(slug:string){if(!isSupabaseDataEnabled())return null;try{return fixture(await rpc("get_public_fixture_page_v1",{p_slug:slug}));}catch(error){console.error("Permanent fixture page could not be loaded",error instanceof Error?error.message:"unknown error");return null;}}
async function loadMatchupPage(slug:string,eventId?:string){if(!isSupabaseDataEnabled()||eventId&&!UUID_RE.test(eventId))return null;try{return fixture(await rpc("get_public_matchup_page_v1",{p_slug:slug,p_event_id:eventId??null}));}catch(error){console.error("Permanent matchup page could not be loaded",error instanceof Error?error.message:"unknown error");return null;}}
async function loadCompetitionFixtures(sport:string,competition:string){if(!isSupabaseDataEnabled())return[];try{const payload=await rpc("get_public_competition_fixtures_v1",{p_sport_slug:sport,p_competition_slug:competition});if(!payload||typeof payload!=="object"||Array.isArray(payload))return[];const list=(payload as Row).fixtures;return Array.isArray(list)?list.map(fixture).filter((item):item is PublicFixture=>Boolean(item)):[];}catch(error){console.error("Competition fixtures could not be loaded",error instanceof Error?error.message:"unknown error");return[];}}
async function loadParticipantFixtures(participantId:string){if(!isSupabaseDataEnabled())return[];try{const payload=await rpc("get_public_participant_fixtures_v1",{p_participant_id:participantId});if(!payload||typeof payload!=="object"||Array.isArray(payload))return[];const list=(payload as Row).fixtures;return Array.isArray(list)?list.map(fixture).filter((item):item is PublicFixture=>Boolean(item)):[];}catch(error){console.error("Participant fixtures could not be loaded",error instanceof Error?error.message:"unknown error");return[];}}
export const getPublicFixturePage=cache(loadFixturePage);
export const getPublicMatchupPage=cache(loadMatchupPage);
