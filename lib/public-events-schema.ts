...
function requiredString(
  ...
  if (typeof value !== "string") {
    throw new PublicEventsPayloadError(`${field} must be a string`);
  }

  const normalized = value.trim();
  if (!normalized || normalized.length > maxLength) {
    throw new PublicEventsPayloadError(`${field} has an invalid length`);
  }
...
function localPath(value: unknown, field: string): string {
  ...
  if (url.origin !== "https://watchtvsport.invalid") {
    throw new PublicEventsPayloadError(`${field} must stay on WatchTVSport`);
  }
...
function slug(value: unknown, field: string): string {
  ...
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(input)) {
    throw new PublicEventsPayloadError(`${field} must be a normalized slug`);
  }
  return input;
}
...
function parseParticipant(value: unknown, field: string): Participant | undefined {
  ...
  if (!isRecord(value)) {
    throw new PublicEventsPayloadError(`${field} must be an object`);
  }

  const type = requiredString(value.type, `${field}.type`, 40);
  const visualType = requiredString(value.visualType, `${field}.visualType`, 40);
  const allowedTypes = new Set(["national_team", "club", "player", "event"]);
  const allowedVisualTypes = new Set(["flag", "crest", "player", "generic"]);

  if (!allowedTypes.has(type) || !allowedVisualTypes.has(visualType)) {
...
function parseLanguages(value: unknown, field: string): string[] | undefined {
  ...
  if (!Array.isArray(value) || value.length > 30) {
    throw new PublicEventsPayloadError(`${field} must be a short array`);
  }

  return value.map((language, index) =>
    requiredString(language, `${field}[${index}]`, 40)
  );
}
...
function parseBroadcast(value: unknown, field: string): BroadcastInfo {
  ...
  const broadcastType = requiredString(
    value.broadcastType,
    `${field}.broadcastType`,
    20
  );
  ...
  if (access !== "Free" && access !== "Paid") {
    throw new PublicEventsPayloadError(`${field}.access must be Free or Paid`);
  }
  if (!allowedBroadcastTypes.has(broadcastType)) {
    throw new PublicEventsPayloadError(`${field}.broadcastType is unsupported`);
  }
  if (value.coverageStatus !== "confirmed") {
    throw new PublicEventsPayloadError(`${field} must be confirmed`);
  }

  const coverageType = optionalString(
    value.coverageType,
    `${field}.coverageType`,
    20
  );
  if (coverageType && !["full", "partial", "unknown"].includes(coverageType)) {
    throw new PublicEventsPayloadError(`${field}.coverageType is unsupported`);
  }

  return {
    countryCode: requiredString(value.countryCode, `${field}.countryCode`, 12)
      .toLowerCase(),
    ...
    affiliateUrl:
      value.affiliateUrl === null || value.affiliateUrl === undefined
        ? undefined
        : httpsUrl(value.affiliateUrl, `${field}.affiliateUrl`),
    sourceName: optionalString(value.sourceName, `${field}.sourceName`, 240),
    sourceUrl:
      value.sourceUrl === null || value.sourceUrl === undefined
        ? undefined
        : httpsUrl(value.sourceUrl, `${field}.sourceUrl`),
    lastChecked: optionalString(value.lastChecked, `${field}.lastChecked`, 40),
    notes: optionalString(value.notes, `${field}.notes`, 2_000),
    commentaryLanguages: parseLanguages(
      value.commentaryLanguages,
      `${field}.commentaryLanguages`
    ),
    coverageType: coverageType as BroadcastInfo["coverageType"],
    coverageStatus: "confirmed",
    broadcastType: broadcastType as BroadcastInfo["broadcastType"],
    accessConditions: optionalString(
...
function parseEvent(value: unknown, index: number): EventData {
  ...
  if (!Array.isArray(rawBroadcasts) || rawBroadcasts.length > MAX_BROADCASTS_PER_EVENT) {
    throw new PublicEventsPayloadError(`${field}.broadcasts exceeds its limit`);
  }

  const status = optionalString(value.status, `${field}.status`, 20);
  if (status && !["scheduled", "live", "finished"].includes(status)) {
    throw new PublicEventsPayloadError(`${field}.status is unsupported`);
  }
  ...
  return {
    ...
    broadcasts: rawBroadcasts.map((broadcast, broadcastIndex) =>
      parseBroadcast(broadcast, `${field}.broadcasts[${broadcastIndex}]`)
    ),
  };
}
...
export function parsePublicEventsPayload(value: unknown): PublicEventsPayload {
  if (!isRecord(value)) {
    throw new PublicEventsPayloadError("payload must be an object");
  }
  if (value.schemaVersion !== PUBLIC_EVENTS_SCHEMA_VERSION) {
    throw new PublicEventsPayloadError("payload schema version is unsupported");
  }
  if (!Array.isArray(value.events) || value.events.length > MAX_PUBLIC_EVENTS) {
    throw new PublicEventsPayloadError("events must be a bounded array");
  }
  ...
  for (const event of events) {
    if (identities.has(event.id)) {
      throw new PublicEventsPayloadError(`duplicate event id: ${event.id}`);
    }
    if (paths.has(event.detailPath)) {
      throw new PublicEventsPayloadError(
        `duplicate event detail path: ${event.detailPath}`
      );
    }
